...
const VAPID_PUBLIC_KEY = Deno.env.get("VAPID_PUBLIC_KEY") ?? ""const VAPID_PRIVATE_KEY = Deno.env.get("VAPID_PRIVATE_KEY") ?? ""
...
async function makeJwt(audience) {
...
async function sendPush(endpoint, p256dh, auth, payload) {
  ...
  const receiverPub = await crypto.subtle.importKey("raw", fromB64url(p256dh), { name: "ECDH", namedCurve: "P-256" }, false, [])
